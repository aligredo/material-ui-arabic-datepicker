      ...
      return _react2.default.createElement(
        ...
        _react2.default.createElement(
          ...
          _react2.default.createElement(_ToolbarButton2.default, {
            ...
            selected: !showYearSelection,
            label: arabicFormat(utils.getDatePickerHeaderText(this.date))
          })
        ),
        this.props.children,
...
var arabicFormat = function (date) {
  var formatted = "";
  var num = date.slice(-2);

  if(num.slice(0) !== ' '){
	num = " " + num;
  }
  var day = date.slice(0,3);
  var spacePos = date.search(" ");
  var monthSliced = date.slice(spacePos+1 ,spacePos+4);

  switch(day){
	case "Sun": formatted = formatted.concat(" اﻷحد");break;
	case "Mon": formatted = formatted.concat(" اﻷثنين");break;
	case "Tue": formatted = formatted.concat(" الثلاثاء");break;
	case "Wed": formatted = formatted.concat(" اﻷربعاء");break;
	case "Thu": formatted = formatted.concat(" الخميس");break;
	case "Fri": formatted = formatted.concat(" الجمعة");break;
	case "Sat": formatted = formatted.concat(" السبت");break;
  }

	switch(monthSliced)
{
	case "Jan" : return  formatted.concat("، " +num+" يناير ");
	case "Feb" : return  formatted.concat("، " +num+" فبراير ");
	case "Mar" : return  formatted.concat("، " +num+" مارس ");
	case "Apr" : return  formatted.concat("، " +num+" أبريل ");
	case "May" : return  formatted.concat("، " +num+" مايو ");
	case "Jun" : return  formatted.concat("، " +num+" يونيو ");
	case "Jul" : return  formatted.concat("، " +num+" يوليو ");
	case "Aug" : return  formatted.concat("، " +num+" أغسطس ");
	case "Sep" : return  formatted.concat("، " +num+" سبتمبر ");
	case "Oct" : return  formatted.concat("، " +num+" أكتوبر ");
	case "Nov" : return  formatted.concat("، " +num+" نوفمبر ");
	case "Dec" : return  formatted.concat("، " +num+" ديسمبر ");
